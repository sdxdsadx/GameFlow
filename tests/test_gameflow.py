...
class GameFlowTests(unittest.TestCase):
    ...
    def test_resource_update_marker_detection_is_removed(self):
        root = Path(__file__).resolve().parents[1]
        config_text = (root / "config" / "workflow.json").read_text(encoding="utf-8")
        runner_text = (root / "gameflow" / "runners.py").read_text(encoding="utf-8")
        self.assertNotIn('"update_markers"', config_text)
        self.assertNotIn("_find_update_marker", runner_text)
        self.assertNotIn("_needs_update", runner_text)
