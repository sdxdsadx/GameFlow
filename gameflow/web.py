...
<header class="topbar"><div class="brand"><div class="brand-mark">✦</div><div><h1>GameFlow</h1><small>DIMENSIONAL OPERATIONS</small></div></div><div class="top-actions"><div id="clock" class="clock">SYNC --:--:--</div><button class="icon-btn" onclick="toggleTheme()" title="切换昼夜主题">☾</button></div></header>
...
<main class="layout"><div class="stack"><section class="card"><div class="card-title"><div><span class="section-code">SQUAD FORMATION</span><h3>每日任务编队</h3></div><span class="muted small">按住任务卡片直接拖拽 · ↑↓ 也可微调</span></div><div id="tasks"></div><div class="toolbar"><label class="parallel-control">同步出击上限 <select id="parallel"><option value="1">1 队 · 顺序执行</option><option value="2">2 队 · 并行执行</option></select></label><button id="startDaily" class="primary" onclick="runDaily()">✦ 开始每日作战</button><button class="danger" onclick="stopAll()">终止行动</button></div></section></div>
