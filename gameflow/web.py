...
.test-action{background:linear-gradient(135deg,#188da7,#5b71e8)}.top-parallel{display:flex;align-items:center;gap:7px;padding:5px 6px 5px 11px;border:1px solid var(--line);border-radius:12px;background:var(--panel);color:var(--muted);font-size:11px}.top-parallel select{padding:6px 25px 6px 8px}.quick-nav{display:flex;gap:5px}.quick-nav a{padding:9px 10px;border:1px solid var(--line);border-radius:10px;color:var(--muted);font-size:11px;text-decoration:none;background:var(--panel)}.top-actions{flex-wrap:wrap;justify-content:flex-end}.hero-telemetry{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin-top:13px}.hero-metric{min-width:0;padding:8px 10px;border:1px solid rgba(142,151,255,.14);border-radius:11px;background:rgba(7,9,29,.28)}.hero-metric b{display:block;color:var(--muted);font-size:9px;letter-spacing:1px}.hero-metric span{display:block;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}.trial-note{color:var(--cyan);font-size:10px}
...
async function refresh(){
 try{
  let d=await api('/api/status');workflows=d.workflows.filter(x=>x.id!=='self_test'&&!x.test_only);
  ...
  $('progressText').textContent=`${finished} / ${total}`;$('progressFill').style.width=(total?Math.min(100,finished/total*100):0)+'%';$('startDaily').disabled=busy;$('qqTrial').disabled=busy;let qq=currentStates.qq_reader_trial||{};$('qqTrial').textContent=qq.running?'阅 测试运行中':'阅 QQ阅读测试';renderHistory(d.runs||[])
...
async function runQQTrial(){await runOne('qq_reader_trial',true)}
